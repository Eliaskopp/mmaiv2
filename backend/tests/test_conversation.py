...
async def _create_conversation(
    ...
    payload = {**overrides}
    resp = await client.post("/api/conversations", json=payload, headers=headers)
...
async def test_list_conversations_empty(client: AsyncClient):
    data = await _register(client)
    resp = await client.get("/api/conversations", headers=_auth(data))
...
async def test_list_conversations_own_only(client: AsyncClient):
    ...
    resp = await client.get("/api/conversations", headers=_auth(user1))
    body = resp.json()
    assert body["total"] == 1
...
async def test_list_conversations_pagination(client: AsyncClient):
    ...
    resp = await client.get("/api/conversations?limit=2&offset=0", headers=headers)
    body = resp.json()
    assert len(body["items"]) == 2
    assert body["total"] == 5

    resp2 = await client.get("/api/conversations?limit=2&offset=4", headers=headers)
    body2 = resp2.json()
...
async def test_delete_conversation_soft_delete(client: AsyncClient):
    ...
    get_resp = await client.get(f"/api/conversations/{conv['id']}", headers=headers)
...
async def test_create_conversation_unauthenticated(client: AsyncClient):
    resp = await client.post("/api/conversations", json={"title": "Nope"})
...
@pytest.mark.asyncio
async def test_send_message_stub_response(client: AsyncClient):
    data = await _register(client)
    headers = _auth(data)
    conv = await _create_conversation(client, headers)

    messages = await _send_message(client, headers, conv["id"], "What should I train?")
    assert "stub mode" in messages[1]["content"].lower()
...
async def test_get_conversation_not_found(client: AsyncClient):
    data = await _register(client)
    fake_id = str(uuid.uuid4())
    resp = await client.get(f"/api/conversations/{fake_id}", headers=_auth(data))
    assert resp.status_code == 404
...
async def test_get_conversation_other_user(client: AsyncClient):
    ...
    resp = await client.get(
        f"/api/conversations/{conv['id']}", headers=_auth(user2),
    )
    assert resp.status_code == 404
...
async def test_get_conversation_soft_deleted(client: AsyncClient):
    ...
    headers = _auth(data)
    conv = await _create_conversation(client, headers)
    await client.delete(f"/api/conversations/{conv['id']}", headers=headers)
...
async def test_list_messages_pagination(client: AsyncClient):
    ...
    resp = await client.get(
        f"/api/conversations/{conv['id']}/messages?limit=2&offset=0",
        headers=headers,
    )
    body = resp.json()
    assert len(body["items"]) == 2
...
async def test_list_messages_other_user(client: AsyncClient):
    ...
    resp = await client.get(
        f"/api/conversations/{conv['id']}/messages", headers=_auth(user2),
    )
    assert resp.status_code == 404
